  ...
  findOverlappingValueSets: ->
    ...
    for curValueSet in summaryValueSets
      curValueSet.codes.fullCollection.each (curCode) =>
        ...
        if codeToVs[workingCode] == null
          codeToVs[workingCode] = []
        else
          for overlap in codeToVs[workingCode]
            orderedOverlaps = _.sortBy([overlap, vsAndCode], function(overlap) {
                return overlap['valueSet'].cid
              })
            overlapKey = orderedOverlaps[0]['valueSet'].cid + "_" + orderedOverlaps[1]['valueSet'].cid
            if overlapCodes[overlapKey] == null
              overlapCodes[overlapKey] = []
            if overlapValueSets[overlapKey] == null
              overlapValueSets[overlapKey] = orderedOverlaps
            overlapCodes[overlapKey].push(curCode)
        codeToVs[workingCode].push(vsAndCode)
    
    for overlapKey in overlapValueSets.keys()
